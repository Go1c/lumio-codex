...
| D-25 ✅已修+⏳待真机复验（2026-08-16 补记，用户 Windows 真机报告：自选目录装的官方 Codex 被手动删除后，管理器不重启就点「启动」反复报「未检测到官方应用 / CODEX_APP_NOT_FOUND」，问「是不是应该重新下载」）：设计答案与实现缺口各一半。**设计上就该引导重装**——应用不在时首页回到「未检测到官方应用」+ 主按钮「安装并启动官方 Codex」，ADR-0006 已把「自定义路径失效 → 回落自动扫描 → 重新触发安装引导」记为诚实降级；重启管理器即可拿到该状态（当前线上构建亦然）。缺口在于**不重启就永远翻不回去**：`codex-app-changed` 事件只接受非空应用（D-3 时按「手选补救」设计），首页 codexApp 是安装/手选时记下的内存态，应用被删后既无法清空也没有任何重检测时机，用户被困在只会报错的「启动」按钮上。修复：① 事件拓宽为 `app: LumioCodexApp | null`，reducer 对安装进度段的 `event.app.path` 判空（`?? null`）——null 即翻回安装引导（按钮从「启动」变「安装并启动官方 Codex」）；② `HomeView.launch()` 对 `CODEX_APP_NOT_FOUND` 失败分支追加一次 `detectCodexApp()` 重检测并把结果（含 null）派发回状态机——应用真没了就地翻回安装引导，误报（应用其实在）则自动纠正路径。测试：`a_vanished_app_can_be_cleared_back_to_the_install_guidance`（先红：null 在 reducer 的 `event.app.path` 处直接崩溃）+ tsc 全量校验事件可空签名。前端 156 测试 + tsc 全绿。**待真机复验**：删掉自选目录里的官方应用 → 点「启动」报一次错后首页自动翻回安装引导 → 重装成功 | 前端 `state.ts`（事件可空 + 判空）、`LumioApp.tsx`（回调签名）、`views/HomeView.tsx`（启动失败重检测） |
| D-24 ✅已修+⏳待真机复验（2026-08-16 补记，用户 Windows 真机报告：自选目录安装官方 Codex 完成后**无法启动**，提示「未检测到官方应用」——D-23 便携自选目录路线首次真机验收即翻车）：根因是 D-23 的检测接线只做了一半。`detect_existing_app` 的「保存路径优先」只有安装命令（`lumio_install_official_app`）在用，**其余三个消费方仍直连旧的自动扫描 `resolve_codex_app_dir`**——`lumio_bootstrap:307`（重启后首页 codex_app 恒 null → 「未检测到官方应用」）、`lumio_detect_codex_app:605`（装完立即 detect 落空）、`lumio_launch_codex:593`（`session.codex_app` 为空时 `.or_else` 扫不到自选目录 → `CODEX_APP_NOT_FOUND`，正是「无法启动」）。自选目录不在任何自动扫描候选里，对这三个命令完全不可见。附带发现一处竞态：自选路径的持久化发生在 `run_official_app_install` **返回之后**，而 `set_succeeded` 在返回之前就已上报——前端轮询到成功立即发起启动时可能撞上「路径还没落盘」的窗口。修复：① 三处命令全部改道 `official_app_install::detect_existing_app(None)`（手选 session 值 → 保存路径 → 自动扫描的完整链）；② `OfficialAppInstallRequest.write_config` 钩子签名改为 `FnMut(&Path)`（携最终安装路径），live 接线把持久化移进钩子——`set_succeeded` 上报前落盘必然完成；测试闭包随签名机械更新。测试：契约测试 `app_surface_commands_detect_through_the_saved_destination`（三命令必须走 detect_existing_app 且不得出现裸 `app_paths::resolve_codex_app_dir`）+ 核心时序测试 `the_post_install_hook_sees_the_final_path_before_success_is_reported`（钩子时 phase 仍为 Detecting、路径与返回值一致），先红后绿（契约红于 bootstrap 直连扫描；时序红暴露过一次断言反向的测试笔误，已修正）。收口：fmt + `--lib lumio`（175）+ `--test installers`（14）+ manager（77，6 靶全绿）+ `cargo check` 全过。**待真机复验**：自选目录安装 → 装完自动启动成功 → 重启管理器首页仍检测到 → 手动启动正常 | `lumio_commands.rs`（bootstrap/detect/launch 三处改道）、`official_app_install/mod.rs`（write_config 钩子携路径 + 持久化前移） |
| D-23 ✅已实现+⏳待真机复验（2026-08-16 补记，用户 Windows 真机反馈：应用内首次安装官方 Codex 时「不能在最一开始选择安装到哪个盘哪个目录，应该有文件夹选择」）：非缺陷，是安装链路的产品设计缺口，且有平台硬约束——**Windows 主路线是 MSIX 侧载（`Add-AppxPackage`），安装位置由 Windows 系统管理，管理器无法逐应用指定目录**。落地（ADR-0006，计划 `codex/docs/plans/2026-08-16-official-app-install-destination.md`）：① 首次安装点主按钮先弹「选择安装位置」——Windows「标准安装（推荐，MSIX，附『装后可在系统设置→应用中移动』提示）」与「选择安装目录…」（目录选择器 → 便携解压到所选目录）并列，默认标准安装（便携「自退」风险未真机验证，不做默认）；macOS「默认 /Applications」与「选择文件夹…」；② 用户选了目录 → `PlanInput.destination` 强制便携路线（侧载探测可用也不改写）；macOS .app 拷贝目标改为所选目录；③ 安装成功即删 745MB 安装包（失败保留供重试），缓存盘峰值压到下载瞬时；④ 仅自选目录时把最终安装路径落 `state_dir()/official-app-path.json`，`detect_existing_app` 优先读它、失效回落自动扫描——否则重启后误判未安装触发重复安装（与 D-3 手选丢失同族）；⑤ 安全不降级：便携路线仍强制 Authenticode 钉选。IPC `lumio_install_official_app` 增 `destination: Option<String>`（空串视同未选择），契约测试钉签名与透传防静默丢参（同 D-1 手法）。测试：路线强制 / 内联清包成败两态 / 路径存取与回落 / 契约签名 / 前端 `destinationOptions` 三平台形态，先红后绿；收口 fmt + `--lib lumio` + `--test installers` + manager + `npm run check`/`npm test` + spec-lint 全绿。**待真机验收**：Windows 自选目录安装（便携路线首次真实跑通，重点观察无包身份是否自退）→ 重启管理器仍检测到 → 启动 → 聊天；macOS 自选目录同理。known gaps：剩余空间预检、下载缓存迁移目标盘、设置页手动路径持久化（D-3 旧坑）、UX 原型 `#installing` 变体未新增「选择安装位置」弹窗稿（**首轮真机验收发现的启动/检测缺陷见 D-24，已修**） | `official_app_install/plan.rs`（destination 路线）、`mod.rs`（destination 透传/清包/持久化/detect）、`install_path.rs`（新增）、`macos.rs`（dest_root）、`lumio_commands.rs`（IPC）、前端 `install-destination.ts`（新增）/`invoke.ts`/`views/HomeView.tsx`、ADR-0006、UX 规格 §5.5 |
| D-26 ✅已修+⏳待真机复验（2026-08-16 补记，用户报告「开机启动设置未生效」）：两层原因。① **注册路径失配不自愈**——应用被移动/重装到新路径后，macOS 的 LaunchAgent plist / Windows 的 HKCU Run 值仍指向旧路径：登录时 launchd/Run 拉起失效目标（症状即「设置了但没启动」），而 `macos_enabled` 的精确匹配让 UI 显示为「关」，`ensure_default_enabled` 又因偏好已存在（Some(true)）不再注册，用户需手动关再开才能修复；② dev/裸二进制（非 .app bundle）按设计不支持注册（`autostart_supported` → `PREFERENCE_LAUNCH_AT_LOGIN_UNSUPPORTED`），打包环境才可注册。修复（针对①）：bootstrap 钩子升级为三态 `DefaultAction`——从未表达偏好→Register（原行为）；偏好开着但注册指向旧路径→Realign（重写 plist/Run 值指向当前 exe）；注册被用户从系统侧移除→Leave（系统现状权威，不自动恢复，维持原设计）。macOS `macos_registration_is_stale`（plist 存在但内容失配）、Windows `registry_query_indicates_stale`（query stdout 携值数据与当前 exe 比较，纯函数跨平台可测）。测试：`default_action_registers_realigns_and_leaves` 矩阵、macOS stale 三态、registry stdout 三态，先红后绿（红=新函数未实现的编译错）。**待真机复验**：装好的 .app 移动位置 → 重启管理器 → 开机启动自动对齐新路径；另确认用户复现环境是打包 .app（dev 直跑报「当前运行方式不支持开机启动」属预期） | `autostart.rs`（DefaultAction/registration_is_stale/macOS+Windows stale 探测/ensure_default_enabled 重写）、`lumio_commands.rs`（bootstrap 注释） |
| D-27 ✅已修（2026-08-16 补记，用户报告「自动更新未生效」）：更新提醒链路对 prerelease 全盲。`lumio_check_update` 只请求 GitHub `/releases/latest`——该 API **只返回非 prerelease 的正式 Release**，而本仓库 CI 发布作业只创建 prerelease（`pr-build.yml` 的 `--prerelease`），`api.github.com/repos/Go1c/lumio-codex/releases/latest` 实测 404；客户端把 404 静默映射为「无更新」（`update_check.rs` 404 分支），首页横幅永远不出现——正是 `docs/ops/03-release.md` §3.1 预警过的坑。设置页「自动更新」开关 disabled 属产品设计（自动安装未开放，只做提醒+引导），非缺陷。修复：404 时回退请求 `/releases?per_page=10` 列表（按发布时间倒序、含 prerelease，跳过 draft），取第一条可解析的 Release 走同一套 semver 比较与横幅引导；列表也空才回报「无更新」。`reminder_from_release`/`no_update_reminder` 抽出供两条路径共用。测试：列表选首条公开 Release 且跳过 draft、空列表无更新、prerelease 更新触发提醒 + 空 html_url 回落下载页 + 同版本不触发，先红后绿。**运维侧可选**：发一个非 prerelease 的正式 Release 可让旧版本客户端（无回退逻辑）也收到提醒 | `update_check.rs`（回退 + 抽取）、`product.rs`（GITHUB_RELEASES_LIST_API 常量） |
| D-28 ✅已修+⏳待真机复验（2026-08-16 补记，用户报告「选择自定义[安装位置]应该先创建 Codex 文件夹再下载」）：自选安装目录在下载前零准备——目录的选择虽来自系统目录选择器（必然存在），但安装目标（Windows 便携解压目录 / macOS `dest/Codex.app` 父目录）到 **745MB 下载完成后的安装阶段**才被消费，坏目录（只读卷、权限异常、深层缺失）要等全部下完才报 `CODEX_APP_INSTALL_FAILED`，白下一遍。修复：`start_official_app_install_with` 在建缓存目录后、下载前调 `prepare_destination`——`create_dir_all`（先把目录建出来）+ 写探针确认可写，失败立即报 `CODEX_APP_INSTALL_FAILED`（经 D-20 的 `note_background_failure` 守卫正确升级 failed，不会卡 planning）。测试：缺失多级目录先创建、默认路线（None）放行、unix 只读目录失败，先红后绿。**待真机复验**：Windows/macOS 自选目录安装照常走通（预检不误伤正常目录） | `official_app_install/mod.rs`（prepare_destination/dir_is_writable/接线） |
| D-29 ✅已修（2026-08-16 补记，用户报告「余额不刷新」，并要求定时刷新、打开界面主动刷新、加刷新按钮）：在线时余额无任何自动刷新时机——拉取点只有登录/provisioning/手动按钮/充值弹窗轮询/离线重连探测五处，应用常驻托盘（关窗只 hide）后 webview 一直活着但余额永远停在最后一次拉取值；托盘唤起 `show_main_window` 只 show+focus 不触发任何数据刷新；现有手动「刷新」按钮藏在「连接状态」卡片，可发现性差。修复：① **定时刷新**——LumioApp（组件常驻，不受首页/设置切换影响）在 ready-online 阶段每 60s 轮询 `lumio_refresh_account`，静默失败，走现成 `account-refreshed` 事件；② **窗口唤起刷新**——Rust `show_main_window` 发 `lumio://window-shown` 事件，前端监听后按「距上次成功同步 ≥10s」节流补刷（`shouldAutoRefresh` 纯函数，防托盘反复点击打爆 `/api/v1/auth/me`）；③ **余额卡片刷新按钮**——「余额与套餐」卡片内紧挨充值按钮加「刷新余额」，复用 refreshing 态与 canRefresh 守卫。新模块 `account-refresh.ts`（常量 + 节流判定 + 事件名，与 Rust 侧逐字一致）。测试：`shouldAutoRefresh` 五态（未同步/不可解析/间隔内/超间隔/周期≥最小间隔）+ 事件名钉死，先红后绿；前端 162 测试全过 | 前端 `account-refresh.ts`（新增）/`account-refresh.test.ts`（新增）/`LumioApp.tsx`（定时 + 事件监听）/`views/HomeView.tsx`（余额卡片按钮）、Rust `lib.rs`（show_main_window emit，Emitter import） |
| D-30 ✅已实现+⏳待真机复验（2026-08-16 补记，用户澄清更新需求：不要后台自动安装，但要有常驻提示，点击后在应用内下载更新、再手动完成安装）：此前「查看更新」只是 `open_in_browser` 打开 Releases 页，用户要在浏览器里自己找包。落地为「提示 → 点击 → 应用内下载 → 打开安装向导」：① **资产选择谓词修正**——遗留 `update.rs` 的安装包谓词只认 CodexPlusPlus 命名（要求文件名含 "plus"、setup 后缀直接结尾），对本仓库 CI 的 `LumioCodex-<ver>-windows-x64-setup-internal-unsigned.exe` / `LumioCodex-<ver>-macos-<arch>-internal-unsigned.dmg` 全部失配（永远选不出资产）；放宽为认 "codex" + setup/installer 标记 + arch token 兼容「后跟点」与「后跟连字符」两种形态（旧命名兼容不回退）；② **core 新增 `download_and_launch_update`**——复用 D-27 的 `fetch_latest_release`（含 prerelease 列表回退），校验资产后走遗留 `perform_update`（下载 Release 安装包到缓存目录 `updates/` + `launch_installer`：Windows 运行安装器 / macOS `open` DMG），安装由用户在向导里完成；新错误码 `UPDATE_ASSET_UNAVAILABLE` / `UPDATE_DOWNLOAD_FAILED`；③ **IPC `lumio_download_update`** 入 lumio 命令白名单（契约测试同步）；④ **前端提示面（按用户后续澄清定稿：右下角弹窗 + 绿色标记，替换首轮的首页横幅方案）**——检测到新版本时**右下角弹出常驻通知卡**（`lumio-update-pop`，与右上角 toast 对称、220ms 滑入、绿色 accent，「稍后」只收本卡）；**绿色小标记常驻**三处：顶部导航「设置」按钮角标绿点（`lumio-nav-dot`）、footer 版本号旁「有新版本 vX」绿色入口、设置页「自动更新」行下方插入「发现新版本 vX · 可更新」条目（绿色图标 + 「立即更新」按钮，绿点的落点）；三处入口与弹窗按钮都触发 `onUpdateRequested`（busy 态「正在下载…」）；下载成功 toast「已打开更新包，请按安装向导完成更新」；设置页说明改为「不会在后台自动安装；有新版本时右下角会弹出提示、设置入口有绿色标记，点击即可下载更新」；⑤ **弹窗频率闸门（同日用户二次澄清：弹窗不要频繁，每天一次；用户不更新可忽略该版本、下个版本再提示）**——新增 `lumio/update_notice.rs` 本地偏好（`state_dir()/update-notice.json`：`dismissed_version` + `last_notice_day` epoch 天）：**「稍后」= 持久忽略该版本**（`lumio_dismiss_update`，跨重启生效，更新的版本出现才恢复弹窗）；**同一天最多弹一次**（弹窗真正渲染时前端调 `lumio_update_notice_shown` 记当天，未表达忽略的版本第二天可再提示）；`lumio_check_update` 下发 `notice_muted` 静默位，**只控制弹窗**——常驻绿色 Update 标记三处不受影响；测试 `update_notice` 六用例（含「版本忽略优先于每日闸门」「损坏偏好回落默认」）+ 命令面契约（4 条更新命令）+ shell-copy 契约（noticeMuted/dismissUpdate/updateNoticeShown），零新依赖（「每天」按 UTC epoch 天）。**待真机复验**：同版本当日重启不再弹、点「稍后」后重启不再弹、发新版本恢复弹窗、绿标三处全程常驻。测试（整体）：updater 谓词新用例（Lumio internal-unsigned 命名 + 裸 exe 拒选 + macOS 按 arch 选 DMG，先红）、`release_has_downloadable_asset` 三态（先红）、`update_notice` 六用例、命令面契约、shell-copy 契约。收口：fmt + core lib 394 + installers 14 + updater 9 + manager 77 + 前端 162 + tsc + vite build 全绿。known gaps：下载无进度条（安装包几十 MB，按钮 busy 态兜底）、未做下载包校验（安装器自带签名校验；GitHub https 直链） | `update.rs`（资产谓词）、`lumio/update_notice.rs`（新增）、`lumio/update_check.rs`（fetch_latest_release 抽取 + download_and_launch_update + notice_muted）、`lumio_commands.rs`（命令 + payload）、`lib.rs`（注册）、前端 `types.ts`/`invoke.ts`/`errors.ts`/`LumioApp.tsx`（弹窗 + 绿点 + footer 入口 + 下载动作 + 频率闸门）/`views/HomeView.tsx`（移除横幅）/`views/SettingsView.tsx`（更新条目行 + 文案）/`lumio-shell.css`（update-pop/nav-dot/is-update 变体）/`shell-copy.test.ts`（契约） |
