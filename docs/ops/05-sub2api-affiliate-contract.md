...
| `invitee_recharge_total` | float | 被邀人**可返点付费订单**金额累计：含余额充值订单与外部支付订阅订单；不含手工余额兑换码、余额内扣订阅（实测 0） |
| `effective_rebate_rate_percent` | float? | 当前实际生效比例（实测 1 = L1）；未配置或未达标时为 `null` |
...
| 字段 | 类型 | 语义 |
| --- | --- | --- |
| `rebate_freeze_hours` | int | 返利冻结小时，`0` = 不冻结 |
| `rebate_duration_days` | int | 邀请关系有效天数，`0` = 永久有效 |
| `rebate_per_invitee_cap` | float | 单被邀人返利上限（元），`0` = 无上限 |
...
- 冻结/有效期/单被邀上限/注册赠送文案**一律读 `GET /user/aff` 的 `data.rules`**（§3.1），
  不得硬编码；`aff_frozen_quota > 0` 才渲染冻结区块。部署前生产无 `rules` 字段——
  门户须容忍缺失并按「未知」隐藏对应文案（缺失 ≠ 0，0 有明确语义）。
