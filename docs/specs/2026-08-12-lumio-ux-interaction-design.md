...
- **动作区**（按危险程度排序）：
  ...
  2. 「恢复本机配置」（secondary，警示色）：点击弹二次确认「将撤销由 Lumio 管理的配置字段并恢复接管前内容，你的其他本机设置不受影响。」确认后执行并显示结果。
...
| 遥测 | 开关（默认关） | 开启时弹说明层，列出仅采集的四类字段（版本 / 平台 / 阶段 / 脱敏错误码），确认后才生效 |
| 日志导出 | 「导出」按钮 | 导出中 loading；完成后 toast「已导出到 <路径>」+「在访达 / 资源管理器中显示」 |
| 配置恢复 | 「恢复」按钮（警示色） | 与修复页动作 2 完全一致的二次确认与结果反馈 |
