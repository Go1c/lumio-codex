...
门户 `/account` 登录后用页签分栏：账户 / 余额 / 开通记录 / 邀请返利。`#orders`（及旧锚 `#claude-orders`）落地开通记录页签。开通记录页签读控制面 `GET /api/v1/me/entitlement` 与 `GET /api/v1/billing/orders`（信封走 `lib/ccControl.ts`，不复用 `@lumio/auth`）：展示当前有效期 / 剩余天数，以及开通账单表格。pending 写「处理中，请勿重复支付」，并提供「刷新开通状态」：`POST /api/v1/billing/orders/{orderNo}/resume` 用原 `order_no` 重放 debit（幂等键仍是订单号），不换新单、不在门户卖套餐。none / expired 且无处理中订单时引导回桌面 Claude Tab。时长一律用服务端 `days_left` / `expires_at`（本地日历日），不要用 `account.planLabel` 或 `created_at+30`。桌面 Claude Tab 的「开通记录」打开 `https://bestcodex.app/account#orders`，不在客户端内嵌账单列表。
