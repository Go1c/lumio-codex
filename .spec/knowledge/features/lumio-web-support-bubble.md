...
description: 官网右下角客服气泡：QQ 群号可复制、飞书群外链，三站 SiteShell 共用——改社群入口或气泡交互时查
metadata:
  type: doc
  status: 已交付
---
...
三站右下角一枚聊天气泡，打开后只提供 QQ 群号（点击复制）与飞书群外链。没有在线对话、没有工单表单。
...
- **实现面**：`@lumio/ui` 的 `SupportBubble` 挂在 `SiteShell` 末尾，门户 / CC / Codex 自动带上。入口只在 `supportChannels()`（`packages/ui/src/config.ts`）读取，环境变量 `VITE_SUPPORT_QQ_NUMBER` / `VITE_SUPPORT_FEISHU_URL` 可覆盖；写 `off` 可关掉一条通道。
...
- [ADR-0005 客服气泡初版写进前端配置](../../decisions/0005-web-support-bubble-static-channels.md)
- [ADR-0006 QQ 入口是群号不是 URL](../../decisions/0006-web-support-qq-group-number.md)
