...
- **实现面**：
  ...
  - Tauri：仅 `lumio_` 命令白名单（含 `lumio_install_official_app` / `lumio_official_app_status` / `lumio_cancel_official_app` 与 `lumio_claude_*`）；秘密不跨 IPC。Claude 引擎胶水在 `src-tauri/src/claude_*.rs`；fns 以 sidecar / 远端制品接入，不把 `fns-*` 焊进 Codex workspace，构建期由 `scripts/sync-components/stage.mjs` 暂存，真产物不入库
