...
fn patch_server_config(
    ...
    roots.retain(|entry| {
        ...
        let workspace_matches = mapping
            ...
            == Some(workspace_id);
        let root_matches = mapping
            .get(serde_yaml::Value::String("root".to_owned()))
            .and_then(serde_yaml::Value::as_str)
            == Some(remote_root);
        // Remove any prior entry for this workspace-id OR this root path
        // (retries with a new workspace-id but same root must not accumulate)
        !(uid_matches && (workspace_matches || root_matches))
    });
...
fn systemd_quote(value: &str) -> Result<String, DeployFailure> {
    ...
    // systemd unit files do NOT use shell quoting — double quotes are treated
    // as part of the value, causing "path is not absolute" errors. Paths are
    // generated by the deploy pipeline and contain only safe characters.
    Ok(value.to_owned())
}
    ...
    Fut: Future<Output = Result<(), DeployFailure>>,
{
    ...
    match operation().await {
        Ok(()) => {
            eprintln!("[fns-deploy] step={step:?} status=ok");
            progress(DeployProgress {
                project_id: request.project_id.clone(),
                step,
        ...
        Err(mut failure) => {
            eprintln!(
                "[fns-deploy] step={step:?} status=FAILED code={:?}",
                failure.primary
            );
