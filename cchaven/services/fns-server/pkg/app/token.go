...
func (t *tokenManager) Generate(uid int64, nickname, _ string, tokenID int64, nonce string) (string, error) {
	...
	// User JWTs are presented by clients and must remain verifiable after the
	// service is moved or restarted on another machine. The configured secret
	// is the stable trust root; machine binding remains reserved for local
	// share-token encryption below.
...
func (t *tokenManager) Parse(token string) (*UserEntity, error) {
	return parseUserJWT(token, t.config.SecretKey)
}
...
func ParseTokenWithKey(tokenString string, secretKey string) (*UserEntity, error) {
	claims, err := parseUserJWT(tokenString, secretKey)
	if err != nil {
...
// parseUserJWT verifies new tokens with the stable configured secret and then
// accepts tokens issued by the previous machine-bound scheme on this host.
// The fallback preserves existing sessions while new tokens survive migration.
func parseUserJWT(tokenString string, secretKey string) (*UserEntity, error) {
	keys := []string{secretKey, secretKey + "_" + util.GetMachineID()}
	var lastErr error
