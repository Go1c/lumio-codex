...
var wsLogger atomic.Pointer[zap.Logger]
...
func SetWSProductionMode(production bool) {
	wsProductionMode.Store(production)
}
...
func log(t LogType, msg string, fields ...zap.Field) {
	...
	switch t {
	case LogError:
		wsLog.Error(msg, fields...)
	case LogWarn:
		wsLog.Warn(msg, fields...)
	case LogInfo:
		wsLog.Info(msg, fields...)
	case LogDebug:
		wsLog.Debug(msg, fields...)
	}
}
...
func NoteModifyLog(traceID string, uid int64, action string, params ...string) {
	...
	// 结构化日志输出（用于日志聚合和分析）
	if wsLog := wsLogger.Load(); wsLog != nil {
		wsLog.Info("WebSocket action",
	...
	if clients, ok := w.userClients[uidStr]; ok {
		for _, client := range clients {
			if client.TokenID == tokenID {
				log(LogInfo, "WS UpdateTokenScope", zap.Int64("uid", uid), zap.Int64("tokenID", tokenID))
