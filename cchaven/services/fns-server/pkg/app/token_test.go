...
func TestParseTokenWithKeyAcceptsStableConfiguredSecret(t *testing.T) {
	claims := &UserEntity{
		UID:      7,
		Nickname: "stable-user",
	}
	token, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).
		SignedString([]byte("stable-secret"))
	if err != nil {
		t.Fatalf("sign stable token: %v", err)
	}

	parsed, err := ParseTokenWithKey(token, "stable-secret")
	if err != nil {
		t.Fatalf("parse stable token: %v", err)
	}
	if parsed.UID != claims.UID || parsed.Nickname != claims.Nickname {
		t.Fatalf("parsed claims = %#v, want %#v", parsed, claims)
	}

	legacyToken, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).
		SignedString([]byte("legacy-secret_" + util.GetMachineID()))
	if err != nil {
		t.Fatalf("sign legacy token: %v", err)
	}
	if _, err := ParseTokenWithKey(legacyToken, "legacy-secret"); err != nil {
