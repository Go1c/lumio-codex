...
func registerAPIRoutes(r *gin.Engine, appContainer *app.App, wss *pkgapp.WebsocketServer, uni *ut.UniversalTranslator, workspaceV2Servers ...*websocket_router.WorkspaceV2Server) {
	cfg := appContainer.Config()
	api := r.Group("/api")
	{
		if len(workspaceV2Servers) > 0 && workspaceV2Servers[0] != nil {
			api.GET("/user/workspace-sync/v2", workspaceV2Servers[0].Run())
		}
