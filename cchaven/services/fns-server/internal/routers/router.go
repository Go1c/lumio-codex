...
func NewRouter(frontendFiles embed.FS, appContainer *app.App, uni *ut.UniversalTranslator) *gin.Engine {
	...
	workspaceV2Server := websocket_router.NewWorkspaceV2Server(
		appContainer,
		websocket_router.NewWorkspaceV2AccessPolicy(cfg.Workspace),
		gws.ServerOption{ResponseHeader: responseHeader},
	)
	appContainer.SetWorkspaceV2Lifecycle(workspaceV2Server)
	...
	// Register API routes
	// 注册 API 路由
	registerAPIRoutes(r, appContainer, wss, uni, workspaceV2Server)
