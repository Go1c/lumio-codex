...
type App struct {
	...
	*Services
	...
	workspaceV2      WorkspaceV2Lifecycle
}

// WorkspaceV2Lifecycle is the shutdown contract owned by the workspace v2 route.
type WorkspaceV2Lifecycle interface {
	Close()
	WaitAllClosed(timeout time.Duration) error
}

// WorkspaceV2TokenLifecycle is the token invalidation contract owned by the
// workspace v2 route. It is kept separate from shutdown so lightweight
// lifecycle test doubles do not need to implement token handling.
type WorkspaceV2TokenLifecycle interface {
	KickToken(uid, tokenID int64)
	UpdateTokenScope(uid, tokenID int64, scope string)
}
...
func NewApp(cfg *AppConfig, logger *zap.Logger, db *gorm.DB, efs embed.FS) (*App, error) {
	...
	a.Services, err = initServices(cfg, infra, repos, logger)
	if err != nil {
		if closeErr := a.Close(); closeErr != nil {
			return nil, fmt.Errorf("failed to initialize services: %w (close infra: %v)", err, closeErr)
		}
...
func (a *App) SetWSS(wss *pkgapp.WebsocketServer) {
	...
	if a.wss != nil && a.Services != nil && a.Services.TokenService != nil {
		a.Services.TokenService.SetSyncHandler(func(uid int64, tokenID int64, scope string, kick bool) {
			a.syncTokenChange(uid, tokenID, scope, kick)
		})
	}
}

func (a *App) syncTokenChange(uid, tokenID int64, scope string, kick bool) {
	if a == nil {
		return
	}
	if kick {
		...
		if lifecycle, ok := a.workspaceV2.(WorkspaceV2TokenLifecycle); ok {
			lifecycle.KickToken(uid, tokenID)
		}
		return
	}
	if a.wss != nil {
	...
	if lifecycle, ok := a.workspaceV2.(WorkspaceV2TokenLifecycle); ok {
		lifecycle.UpdateTokenScope(uid, tokenID, scope)
	}
}
...
// SetWorkspaceV2Lifecycle registers the workspace v2 server for graceful shutdown.
func (a *App) SetWorkspaceV2Lifecycle(server WorkspaceV2Lifecycle) {
	if a == nil {
		return
	}
	a.workspaceV2 = server
}

// GetWorkspaceV2Lifecycle returns the workspace v2 lifecycle owner.
func (a *App) GetWorkspaceV2Lifecycle() WorkspaceV2Lifecycle {
	if a == nil {
		return nil
	}
	return a.workspaceV2
}
...
func (a *App) Shutdown(ctx context.Context) error {
	a.shutdownOnce.Do(func() {
		a.shutdownResult = a.shutdown(ctx)
	})
...
func (a *App) shutdown(ctx context.Context) error {
	...
	if a.workspaceV2 != nil {
		a.logger.Info("Closing workspace v2 connections...")
		a.workspaceV2.Close()
		if err := a.workspaceV2.WaitAllClosed(10 * time.Second); err != nil {
			a.logger.Warn("Workspace v2 connection shutdown error", zap.Error(err))
			errs = append(errs, fmt.Errorf("workspace v2 shutdown: %w", err))
		} else {
			a.logger.Info("All workspace v2 connections closed")
		}
	}
