...
func TestLoadConfigLoadsWorkspace(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.yaml")
	err := os.WriteFile(configPath, []byte(`
workspace:
  blob-path: /tmp/workspace-blobs
`), 0644)
	require.NoError(t, err)

	cfg, _, err := LoadConfig(configPath)
	require.NoError(t, err)
	require.Equal(t, "/tmp/workspace-blobs", cfg.Workspace.BlobPath)
	require.Equal(t, uint64(5_368_709_120), cfg.Workspace.MaxBytes)
}

func TestLoadConfigRejectsZeroWorkspaceMaxBytes(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.yaml")
	err := os.WriteFile(configPath, []byte(`
workspace:
  max-bytes: 0
`), 0644)
	...
	require.ErrorContains(t, err, "validate workspace config failed")
}
