...
type Services struct {
	...
	WorkspaceBlobStore   service.WorkspaceBlobStore
	WorkspaceSyncService service.WorkspaceSyncService
}

// initServices initializes all services
func initServices(cfg *AppConfig, infra *Infra, repos *Repositories, logger *zap.Logger) (*Services, error) {
	if cfg == nil {
		return nil, fmt.Errorf("app config is nil")
	}
	if err := cfg.Workspace.Validate(); err != nil {
		return nil, fmt.Errorf("validate workspace config: %w", err)
	}
	...
	s.WorkspaceBlobStore = service.NewWorkspaceBlobStore(repos.WorkspaceRepo, &cfg.Workspace)
	if initializer, ok := s.WorkspaceBlobStore.(interface{ InitError() error }); ok {
		if err := initializer.InitError(); err != nil {
			return nil, fmt.Errorf("initialize workspace blob store: %w", err)
		}
	}
	s.WorkspaceSyncService = service.NewWorkspaceSyncService(
		repos.WorkspaceRepo,
		s.WorkspaceBlobStore,
		&cfg.Workspace,
	)
	if initializer, ok := s.WorkspaceSyncService.(interface{ InitError() error }); ok {
		if err := initializer.InitError(); err != nil {
			return nil, fmt.Errorf("initialize workspace sync service: %w", err)
		}
	}
