...
type AppConfig struct {
	...
	Workspace        config.WorkspaceConfig        `yaml:"workspace"`
...
func LoadConfig(f string) (*AppConfig, string, error) {
	...
	// Workspace zero values are validation failures, not omitted values to default again.
	// Workspace 的零值是配置校验错误，不应在第二次默认值填充时被覆盖。
	...
	if err := c.Workspace.Validate(); err != nil {
		return nil, realpath, errors.Wrap(err, "validate workspace config failed")
	}
