...
func captureMCPAuthLog(t *testing.T) *bytes.Buffer {
	t.Helper()
...
func TestMCPMixedAuthWithConfig_DoesNotLogRawAuthorizationScopes(t *testing.T) {
	captured := captureMCPAuthLog(t)
	...
	recorder := runMCPMixedAuthMiddleware(t, config.OAuthConfig{
		...
		UID:         7,
		FNSScope:    "p:rest c:ChatGPT f:scope-log-marker",
		OAuthScopes: []string{"oauth-scope-log-marker"},
	}}, req)

	require.Equal(t, http.StatusForbidden, recorder.Code)
	require.Contains(t, captured.String(), "reason=fns_permission_failed")
	require.NotContains(t, captured.String(), "scope-log-marker")
}

func TestMCPMixedAuthWithConfig_DoesNotLogVerifierErrorDetails(t *testing.T) {
	captured := captureMCPAuthLog(t)
	req := httptest.NewRequest(http.MethodGet, "/api/mcp", nil)
	req.Header.Set("Authorization", "Bearer oauth-token")
	verifierErr := fmt.Errorf("%w: missing %q", internaloauth.ErrInsufficientScope, "oauth-scope-log-marker")

	recorder := runMCPMixedAuthMiddleware(t, config.OAuthConfig{
		Enabled:       true,
		Resource:      "https://example.test/api/mcp",
		DefaultClient: "ChatGPT",
	}, validMCPStaticTokenService(), &fakeMCPOAuthVerifier{err: verifierErr}, req)

	require.Equal(t, http.StatusForbidden, recorder.Code)
	require.Contains(t, captured.String(), "reason=verify_failed")
	require.Contains(t, captured.String(), "insufficient_scope=true")
	require.NotContains(t, captured.String(), "oauth-scope-log-marker")
}
