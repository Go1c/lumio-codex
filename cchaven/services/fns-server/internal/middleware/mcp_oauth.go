...
func MCPMixedAuthWithConfig(secretKey string, tokenService service.TokenService, cfg MCPMixedAuthConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil || claims == nil {
			log.Printf("mcp oauth rejected: reason=verify_failed path=%s client=%q insufficient_scope=%t", c.Request.URL.Path, c.GetHeader("X-Client"), errors.Is(err, internaloauth.ErrInsufficientScope))
		...
		if !pkgapp.VerifyPermissions(scope, "mcp", c.GetHeader("X-Client"), "") {
			log.Printf("mcp oauth rejected: reason=fns_permission_failed path=%s uid=%d client=%q", c.Request.URL.Path, uid, c.GetHeader("X-Client"))
			writeOAuthChallenge(c, http.StatusForbidden, cfg.Resource, "insufficient_scope")
			return
		}
