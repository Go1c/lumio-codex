...
type AdminConfig struct {
	...
	IsReturnSussess               *bool              `json:"isReturnSussess,omitempty" form:"isReturnSussess"`                 // Whether to return success detail (legacy misspelling) // 是否返回成功详情（历史误拼）
	IsReturnSuccess               *bool              `json:"isReturnSuccess,omitempty" form:"isReturnSuccess"`                 // Whether to return success detail // 是否返回成功详情
...
// ReturnSuccessFlag prefers the canonical isReturnSuccess JSON field and
// falls back to the legacy misspelling isReturnSussess.
func (c AdminConfig) ReturnSuccessFlag() *bool {
	if c.IsReturnSuccess != nil {
		return c.IsReturnSuccess
	}
	return c.IsReturnSussess
}
