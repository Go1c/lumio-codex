...
> **⚠️ 身份体系已变更（本文其余部分尚未逐节重写）**
>
> 终端用户的账号已统一收口到 Lumio 账号中心（Sub2API，`api.lumio.games`）。
> 本文里关于「注册 / 邮箱验证 / 登录 / 找回密码 / 改邮箱改口令」的 API 与文案
> **描述的是已下线的实现**：那些端点现在一律返回 410 `auth_migrated`。
> 现行的身份边界、映射表与降级策略以
> [`docs/ops/01-architecture.md`](../../../docs/ops/01-architecture.md)「账号体系」一节为准。
>
> 仍然有效的部分：表结构（`users` 等历史列全部保留）、会话族与桌面 OAuth、
> 订阅与权益、邀请与归因、订单与退款、管理后台（管理员账号 + TOTP 完全未变）。
