...
2. **Microsoft Store.** CI packs `LumioCodex-*-windows-x64-store-unsigned.msix` with `makeappx` from the cargo staging dir. SignPath does **not** sign that MSIX. After Partner Center listing, Microsoft re-signs the package. Store Identity is still a placeholder. Full process (中文): [07-microsoft-store.md](./07-microsoft-store.md). Decision: [0007](../../../.spec/decisions/0007-windows-msix-store-scaffold.md).
...
- [0007](../../../.spec/decisions/0007-windows-msix-store-scaffold.md) — unsigned MSIX track decision
