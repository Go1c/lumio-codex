...
- **布局**（沿用现有 dashboard）：欢迎行（邮箱 + 凭据保护徽章）→ 三张指标卡（余额与套餐 / 连接状态 / 默认模型）→ 行动面板（官方应用检测结果 + 「启动 Codex」按钮）。充值入口在余额与套餐卡内，不在行动面板。
- **在线态交互**：
  - 余额卡显示服务端最终数值 + 套餐摘要，卡内提供「充值」小按钮（与连接状态卡的「刷新」同款）；点击卡片其余区域无跳转。
  ...
  - 「安装并启动官方 Codex」（本机无应用时）：点击先弹「选择安装位置」对话框（D-23 / ADR-0006）——Windows 摆「标准安装（推荐）」（走 MSIX，附说明「安装到 Windows 管理的位置；之后可在 系统设置 → 应用 中『移动』到其他盘」）与「选择安装目录…」（系统目录选择器，解压安装到所选目录）两个并列选项，默认聚焦标准安装；macOS 为「默认位置（/Applications）」与「选择文件夹…」；取消弹窗不开始安装。选定后按钮变「正在安装官方 Codex…」+ 阶段（下载 / 校验 / 安装）并禁用；行动面板底部整行显示安装进度条（D-19）——下载阶段带百分比与「已下载 / 总量」双语（拿不到总量时显示「已下载 MB 数」的往复动画条），校验 / 安装阶段为往复动画条 + 阶段文案；完成后自动启动官方 Codex 并 toast「官方 Codex 已启动」（Windows 走系统应用激活）。安装成功即清理下载的安装包（失败保留供重试）；用户自选目录的安装路径持久化，重启后仍能检测到。进度条动效受 `prefers-reduced-motion` 全局开关收敛。失败 / 取消后原因常驻行动面板（D-20，警示色文案 + 错误码，toast 照发但不作为唯一载体），主按钮即重试入口（重试同样先经「选择安装位置」弹窗）；后台任务在进入主管线前失败（缓存目录不可写等）同样收敛为 failed，不得停在「正在安装」。
  - 「充值」：见 5.6。
- **离线态交互**：
...
| 设置项 | 控件 | 交互细节 |
|--------|------|----------|
| 开机启动 | 开关 | **默认开启（opt-out）**：首次启动（打包构建）自动注册开机启动，用户关闭后落偏好文件、永不自动重开；即点即生效，命令返回后才翻转；失败回弹 + toast 错误（`PREFERENCE_LAUNCH_AT_LOGIN_FAILED` / 开发直跑 `_UNSUPPORTED`）；用户从系统设置移除注册项时如实显示为关，不自动恢复 |
