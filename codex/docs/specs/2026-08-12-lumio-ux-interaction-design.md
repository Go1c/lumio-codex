...
- **在线态交互**：
  ...
  - 「安装并启动官方 Codex」（本机无应用时）：按钮变「正在安装官方 Codex…」+ 阶段（下载 / 校验 / 安装）并禁用；行动面板底部整行显示安装进度条（D-19）——下载阶段带百分比与「已下载 / 总量」双语（拿不到总量时显示「已下载 MB 数」的往复动画条），校验 / 安装阶段为往复动画条 + 阶段文案；完成后自动启动官方 Codex 并 toast「官方 Codex 已启动」（Windows 走系统应用激活）。进度条动效受 `prefers-reduced-motion` 全局开关收敛。失败 / 取消后原因常驻行动面板（D-20，警示色文案 + 错误码，toast 照发但不作为唯一载体），主按钮即重试入口；后台任务在进入主管线前失败（缓存目录不可写等）同样收敛为 failed，不得停在「正在安装」。
  - 「充值」：见 5.6。
- **离线态交互**：
