...
`app/cc-home.html`。左轨：项目列表 + 新建 / 连接新服务器。右工作台默认终端（PTY）。点项目只换右侧。文件 / 冲突 Tab 可见，细稿下一步再画，交互沿用现有 CC 桌面端。
...
已画完（原型可点，见第 8 节对照）：正式图标回刷、Codex 三态、Claude 未订阅 / 说明 / 四步向导 / 左轨+终端。

还没画：

- 工作区「文件 / 冲突」细稿（交互沿用现有 CC 桌面端，只换壳）
- 设置每一组、修复页细稿、帮助中心各篇
